...
def handler(user_ID):
    ...
    columnNames = ['propertyID', 'bathrooms', 'bathroomsFull', 'bathroomsHalf', 'bedrooms',
    ...
    categorical_columns = ['Cooling','Heating']

    for col in categorical_columns:
        merged_data_df[col] = merged_data_df[col].apply(lambda x: 0 if x == "None" else 1)

    categorical_columns = ['Parking','Pets']

    # One-hot encode the specified categorical columns
    merged_data_df = pd.get_dummies(merged_data_df, columns=categorical_columns, drop_first=True)
