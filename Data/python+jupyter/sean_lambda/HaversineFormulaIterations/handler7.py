...
def extract_coordinates_bytes(coord):
    # Check if the coordinate is already a string, and if so, use it directly
    if isinstance(coord, str):
        return extract_coordinates(coord)
    
    # Assume the coordinate is bytes and convert to a readable string
    return extract_coordinates(coord.decode('utf-8'))  # Assuming utf-8 encoding, adjust if needed

def find_nearest_propertyIDs(target_coord, df, num_neighbors=10):
    # Calculate distances using Haversine formula
    df['Distance'] = df['geoLocation'].apply(lambda x: haversine(target_coord, extract_coordinates_bytes(x)))
